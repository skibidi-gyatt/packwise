...
export function validPlacement(
  ...
  if ([0, 1, 2].some((k) => p.pos[k] < -EPS || end(p, k) > bag.dims[k] + EPS))
    return false;
  if (p.dims[0] > bag.opening[0] + EPS || p.dims[2] > bag.opening[1] + EPS)
    return false;
  if (ps.some((q) => overlaps(p, q)) || blockers(p, ps).length) return false;
  if (p.pos[1] > EPS) {
    const supports = ps.filter(
  ...
  const all = [...ps, p],
    loads = topLoads(all);
  return all.every((q) => loads.get(q.item.id)! <= q.item.maxTopLoad + EPS);
}
...
function failure(i: Item, bag: Container, mass: number): string {
  ...
  if (
    !rs.some(
      (r) =>
        r.dims[0] <= bag.opening[0] + EPS && r.dims[2] <= bag.opening[1] + EPS,
    )
  )
    return 'Opening too small for an allowed orientation. Measure the opening, use another bag, or carry separately.';
  ...
  return 'No supported, load-safe insertion found in this search. Try fewer items, verify dimensions, or allow more depth expansion.';
}
function run(
  order: Item[],
  bag: Container,
  ...
  for (const item of order) {
    ...
    if (mass + item.mass <= bag.maxMass + EPS) {
      outer: for (const r of rotations(item)) {
        if (
          r.dims[0] > bag.opening[0] + EPS ||
          r.dims[2] > bag.opening[1] + EPS
        )
          continue;
        for (const pos of points(ps, bag, r.dims)) {
...
export function objective(p: Plan) {
  return (
    p.placements.filter((x) => x.item.required).length * 1e6 +
    p.placements.length * 1e4 +
    p.metrics.quality
  );
}
